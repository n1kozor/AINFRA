...
const StandardDeviceDetails: React.FC<StandardDeviceDetailsProps> = ({ device }) => {
    ...
    const system = (stats?.system || {}) as DeviceStats['system'];
    const cpu = (stats?.cpu || {}) as DeviceStats['cpu'];
    const memory = (stats?.memory || {}) as DeviceStats['memory'];
    const disk = (stats?.disk || []) as DeviceStats['disk'];
    const network = (stats?.network || []) as DeviceStats['network'];
    const processes = (stats?.processes || {}) as DeviceStats['processes'];
                ...
                <Grid container spacing={3}>
                    ...
                    >
                        <Paper sx={{ p: 3, borderRadius: 2, height: '100%' }}>
                            ...
                            {memory?.swap_total && memory.swap_total > 0 && (
                                <Box sx={{ mt: 4 }}>
