...
const SoulChatModal: React.FC<SoulChatModalProps> = ({
  ...
  const getStatusText = () => {
    if (healthScore > 80) return t('chat.excellent');
    if (healthScore > 50) return t('chat.good');
    return t('chat.critical');
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev =>
        prev.map(msg =>
          msg.id === soulResponseId
            ? { ...msg, content: t('chat.errorFetchingResponse'), isTyping: false }
            : msg
        )
      );
    } finally {
              ...
              <Box
                sx={{
                  ...
                  background: `radial-gradient(
                    circle,
                    ${alpha(healthColor, 1.0)} 0%,
                    ${alpha(healthColor, 0.5)} 40%,
