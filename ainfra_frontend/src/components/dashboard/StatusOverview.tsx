...
export const StatusOverview: React.FC<StatusOverviewProps> = ({
  ...
  const getTimeRangeLabel = (range: TimeRangeOption): string => {
    switch (range) {
      case '30m': return t('dashboard.timeRange.thirtyMinutes');
      case '1h': return t('dashboard.timeRange.oneHour');
      case '6h': return t('dashboard.timeRange.sixHours');
      case '24h': return t('dashboard.timeRange.oneDay');
      case '7d': return t('dashboard.timeRange.sevenDays');
      case 'all': return t('dashboard.timeRange.allTime');
      default: return '';
    }
  };
  ...
  const availabilityStatus = useMemo(() => {
    if (!statistics || !statistics.availability_summary) {
      return { rate: 0, status: 'unknown' };
    }
  ...
  const hasRecentErrors = statistics?.recent_errors && statistics.recent_errors.length > 0;
  const hasSlowDevices = statistics?.top_slowest_devices && statistics.top_slowest_devices.length > 0;
  const hasSystemStatus = statistics?.system_status;
  const hasTimestamp = hasSystemStatus && statistics?.system_status.timestamp;
  const activeDevices = statistics?.device_summary?.active_devices || 0;
  const availableDevices = statistics?.availability_summary?.devices_available || 0;
  const unavailableDevices = statistics?.availability_summary?.devices_unavailable || 0;
  const avgResponseTime = statistics?.availability_summary?.avg_response_time_ms?.toFixed(0) || 'N/A';
  const checkInterval = hasSystemStatus ? statistics.system_status.check_interval_minutes : 0;
              ...
              <Box>
                ...
                {hasTimestamp && (
                  <Typography variant="caption" color="text.secondary">
                    {t('dashboard.lastUpdated')}: {formatDistanceToNow(new Date(statistics.system_status.timestamp), { addSuffix: true })}
                  </Typography>
                )}
              </Box>
            </Box>
          ...
          {isLoading ? (
            ...
            <>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={6} md={3}>
                    ...
                    >
                      ...
                      <Typography variant="body2" color="text.secondary" align="center">
                        {t('dashboard.systemStats.availability')}
                      </Typography>
                      <Chip
                        label={t(`dashboard.networkSoul.status.${availabilityStatus.status}`)}
                        sx={{
                  ...
                  <Grid item xs={12} sm={6} md={3}>
                    ...
                    >
                      ...
                      <Typography variant="subtitle2" color="text.primary" gutterBottom>
                        {t('dashboard.deviceStatus.totalDevices')}
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                        {t('dashboard.deviceStatus.available')}: {availableDevices} • {t('dashboard.deviceStatus.unavailable')}: {unavailableDevices}
                      </Typography>
                    </Paper>
                  </Grid>
                  ...
                  <Grid item xs={12} sm={6} md={3}>
                    ...
                    >
                      ...
                      <Typography variant="subtitle2" color="text.primary" gutterBottom>
                        {t('dashboard.systemStats.avgResponseTime')}
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                        ms
                      </Typography>
                  ...
                  <Grid item xs={12} sm={6} md={3}>
                    ...
                    >
                      ...
                      <Typography variant="subtitle2" color="text.primary" gutterBottom>
                        {t('dashboard.systemStats.checkInterval')}
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                        {t(checkInterval > 1 ? 'dashboard.timeRange.minutes' : 'dashboard.timeRange.minute')}
                      </Typography>
                    </Paper>
                  </Grid>
                </Grid>
              ...
              >
                ...
                >
                  <Tab
                    label={t('dashboard.systemStats.availabilityTrend')}
                    icon={<TrendingUpIcon />}
                    iconPosition="start"
                  />
                  <Tab
                    label={t('dashboard.systemStats.recentErrors')}
                    icon={<WarningIcon />}
                    iconPosition="start"
                    sx={{ color: hasRecentErrors ? theme.palette.error.main : 'inherit' }}
                  />
                  <Tab
                    label={t('dashboard.systemStats.slowestDevices')}
                    icon={<SpeedIcon />}
                    iconPosition="start"
                  />
                </Tabs>
                        ...
                        {chartData.length > 0 ? (
                          <ResponsiveContainer width="100%" height="100%">
                            ...
                            >
                              ...
                              <YAxis
                                ...
                                tickLine={{ stroke: alpha(theme.palette.text.primary, 0.2) }}
                                label={{
                                  value: t('dashboard.systemStats.availabilityPercent'),
                                  angle: -90,
                                  position: 'insideLeft',
                              ...
                              <RechartsTooltip
                                ...
                                formatter={(value: any) => [`${Number(value).toFixed(1)}%`, t('dashboard.systemStats.availability')]}
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                            <Typography variant="body1" color="text.secondary">
                              {t('dashboard.systemStats.noData')}
                            </Typography>
                          </Box>
                        )}
                      </Box>
                      ...
                      {chartData.length > 0 && (
                        <Box sx={{
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            ...
                            {`${chartData.length} ${t('dashboard.systemStats.dataPoints')}`}
                          </Box>
                        </Box>
                      )}
                    </>
                    ...
                    <>
                      {hasRecentErrors ? (
                        ...
                        <Box sx={{
                          ...
                          />
                          <Typography variant="body1" color="text.secondary">
                            {t('dashboard.systemStats.noErrors')}
                          </Typography>
                        </Box>
                      )}
                    </>
                    ...
                    <>
                      {hasSlowDevices ? (
                            ...
                            >
                              ...
                              <ListItemText
                                ...
                                secondary={
                                  ...
                                  >
                                    {device.timestamp && formatDistanceToNow(new Date(device.timestamp), { addSuffix: true })}
                                    {device.timestamp && ' • '}
                                    {device.check_method}
                                  </Typography>
                                }
                        ...
                        <Box sx={{
                          ...
                          <Typography variant="body1" color="text.secondary">
                            {t('dashboard.systemStats.noSlowDevices')}
                          </Typography>
                        </Box>
                      )}
                    </>
