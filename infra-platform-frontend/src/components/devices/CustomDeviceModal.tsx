...
import { api } from '../../api';
import { DeviceCreate, CustomDeviceCreate } from '../../types/device';
...
const CustomDeviceModal: React.FC<CustomDeviceModalProps> = ({ open, onClose, onSuccess }) => {
  ...
  // Generate form fields from plugin UI schema
        ...
        {Object.entries(connectionProps).map(([key, prop]: [string, any]) => (
          <Grid item xs={12} sm={6} key={key}>
            <TextField
              ...
              error={required.includes(key) && !deviceData.custom_device?.connection_params?.[key]}
