...
const DeviceCard: React.FC<DeviceCardProps> = ({ device }) => {
  ...
  const handleChatClick = (e: React.MouseEvent) => {
    if (!isAvailable) return; // Prevent chat if device is offline
  ...
  return (
    <motion.div
      ...
      style={{
        position: 'relative',
        margin: '8px',
        transformStyle: 'preserve-3d' // Enable 3D space for child elements
      }}
    >
      {/* Integrated Card + Chat Button container (moves together) */}
      <motion.div
        whileHover={{
          y: -5,
      ...
      >
        {/* Chat Button - Positioned relative to card */}
        <Tooltip
        ...
        >
          <div style={{
            ...
            right: -8,
            zIndex: 20,
            transform: 'translate3d(0,0,10px)' // Place in 3D space
          }}>
            <IconButton
      ...
      {/* Chat Modal - Only rendered when device is online */}
      {isAvailable && (
        <DeviceChatModal
          open={chatOpen}
